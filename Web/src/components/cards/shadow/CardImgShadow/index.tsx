...
const CardImgShadow: React.FC<CardImgShadowProps> = ({ title, actions }) => {
    ...
    return (
        <div ref={scope} className={`${style.cardTitle} ${cardStyle.sizeLarge}`}>
            <section style={DisplayNone} id={DRAGGING_ID} className={style.cardAnimation}>
                <Dragging />
                <div>Drag to the right place</div>
            </section>
            <section id={DOUBLE_CLICK_ID} style={DisplayNone} className={style.cardAnimation}>
                <DoubleClick />
                <div>Double-click for more details</div>
            </section>
            <div id={SHADOW_CARD_TITLE_ID} style={DisplayNone} className={style.cardShadowTitle}>
                {title}
            </div>
        </div>
    );
